...
int new_vec2(lua_State* L)
{
    if (lua_gettop(L) == 0) {
        ...
        apollo::push(L, sf::Vector2<T>(
            apollo::from_stack<T>(L, 1), apollo::from_stack<T>(L, 2)));
    }
    return 1;
}
...
apollo::stack_reference export_vector2(lua_State* L)
{
    ...
    apollo::new_table(L)
        ...
        ("y", FN(APOLLO_MEMBER_GETTER(vec::x)))
        ("set_y", FN(APOLLO_MEMBER_SETTER(vec::x)))
...
#undef OP
    ...
    apollo::new_table(L) // Clock
        ("new", apollo::get_raw_ctor_wrapper<sf::Clock>())
